...
func NewCasbinMiddleware(modelFile string, db *gorm.DB, subFn SubjectFn) (*CasbinMiddleware, error) {
...
func (am *CasbinMiddleware) RequiresRoles(requiredRoles []string, opts ...Option) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		fmt.Println("yang login emailnya =>", sub)
		...
		if actualOptions.logic == AND {
			...
			for _, requiredRole := range requiredRoles {
				...
				if i >= len(actualRoles) || actualRoles[i] != requiredRole {
					...
					fmt.Println("role required", actualRoles, requiredRole)
					c.AbortWithStatus(401)
					return
				}
			}
...
func (am *CasbinMiddleware) GetPermissionsForUser() gin.HandlerFunc {
	return func(c *gin.Context) {
		// Look up current subject.
		sub := am.subFn(c)
		if sub == "" {
		...
		fmt.Println(sub)
		permission, err := casbin.CasbinJsGetPermissionForUser(am.enforcer, sub)
		fmt.Println("aaa", permission)
		if err != nil {
			fmt.Println(err)
		}
